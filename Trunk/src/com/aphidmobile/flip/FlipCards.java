...
public class FlipCards {
	...
	public synchronized boolean handleTouchEvent(MotionEvent event,
		...
		case MotionEvent.ACTION_MOVE:
			...
			Log.i("simon", lastPositionX + "<<<<<\"X\">>>>>" + event.getX()
					+ "<<<<<<<<<<<<<<\"Y\">>>>>>>>>>>>>>" + event.getY());
			
			if(getPageIndexFromAngle(accumulatedAngle)!=lastPageIndex){//控制每次翻一屏
				forward = false;
				break;
			}
			
			Log.i("simon", "<<<<< lastPageIndex >>>>>" + lastPageIndex);
			
			if (Math.abs(deltaX) > controller.getTouchSlop()) {
				setState(STATE_TOUCH);
				forward = deltaX > 0;
			}
			if (state == STATE_TOUCH) {
				// 滑动的距离需要大于最小滑动距离，并且要大于Y轴的滑动距离。如果小于Y轴滑动，视为不翻页，比如scrollView的滑动，此时不处理。
				if (Math.abs(deltaX) > MIN_MOVEMENT
						&& Math.abs(deltaX) > Math.abs(deltaY)) // ignore small
																// movements{
					forward = deltaX > 0;
				else {
					forward = false;
				...
				if (orientationVertical)
					angleDelta = 180 * deltaX / controller.getContentHeight()
							* MOVEMENT_RATE;
				else
					angleDelta = 180 * deltaX / controller.getContentWidth()
							* MOVEMENT_RATE;
