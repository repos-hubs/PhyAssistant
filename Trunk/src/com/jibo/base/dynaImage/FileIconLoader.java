...
public class FileIconLoader implements Callback {
	 ...
	 */
	private final static ConcurrentHashMap<Object, ImageHolder> mImageCache = new ConcurrentHashMap<Object, ImageHolder>();
	/**
	 ...
	 */
	private final ConcurrentHashMap<ImageView, ImageRequest> mPendingRequests = new ConcurrentHashMap<ImageView, ImageRequest>();
	/**
	 * Handler for messages sent to the UI thread.
	 */
	private final Handler mThisThreadHandler = new Handler(this);
	/**
	...
	public static class ImageRequest {
		...
		public Object mPath;// thumbnailPath
		public long mId; // database id

		public ImageRequest(Object path, long id) {
			mPath = path;
			mId = id;
		}
	}
	 ...
	 *            , database id
	 */
	public boolean loadIcon(ImageView view, FileRequest fileRequest) {
		boolean loaded = loadCachedIcon(view, fileRequest.filePath);
		if (loaded) {
			mPendingRequests.remove(view);
		} else {
			ImageRequest p = new ImageRequest(fileRequest.filePath,
					fileRequest.dbId);
	 ...
	 */
	private boolean loadCachedIcon(ImageView view, Object path) {
	...
	private void requestLoading() {
		if (!mLoadingRequested) {
			...
			mThisThreadHandler.sendEmptyMessage(MESSAGE_REQUEST_LOADING);
		}
	}
	...
	private void processLoadedIcons() {
		...
		while (iterator.hasNext()) {
			...
			ImageRequest fileId = mPendingRequests.get(view);
	...
	private class LoaderThread extends HandlerThread implements Callback {
		 ...
		 */
		public boolean handleMessage(Message msg) {
			Iterator<ImageRequest> iterator = mPendingRequests.values()
					.iterator();
			while (iterator.hasNext()) {
				ImageRequest id = iterator.next();
				ImageHolder holder = mImageCache.get(id.mPath);
				Logs.i("-----> ||| " + holder + " "
						+ (holder != null ? holder.state : "-1"));
				if (holder != null && holder.state == ImageHolder.NEEDED) {
					// Assuming atomic behavior
					holder.state = ImageHolder.LOADING;
					if (id.mPath.getClass() == URL.class) {

						byte[] bytes = null;
						try {
							String path = NetConstants.PLACE + "/"
									+ NetUtil.splitPath(((URL) id.mPath));
							File file = new File(path);
							if (!file.getParentFile().exists()) {
								file.getParentFile().mkdirs();
							}
							if (!file.exists()) {
								bytes = NetUtil.getImage((URL) id.mPath);

								Bitmap netbitmap = BitmapFactory
										.decodeByteArray(bytes, 0, bytes.length);// Éú³ÉÎ»Í¼

								FileOutputStream fs = new FileOutputStream(path);
								fs.write(bytes);
								fs.close();
							}
							readPic(id, holder, path, false);
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}

					} else if (!id.mPath.toString().contains("//")) {
						holder.setImage(mContext.getResources().getDrawable(
								R.drawable.news_image_placeholder2));
					} else {
						FileTypeInfo type = FileCategoryHelper
								.getCategoryFromPath(id.mPath);
						switch (type.fc) {
						case Apk:
						...
						case Video:
							boolean isVideo = type.fc == FileCategory.Video;
							readPic(id, holder, null, isVideo);
							break;
						case Music:
							Bitmap image = Utillocal.getImage(
									id.mPath.toString(), null, mContext);
							if (image != null) {
			...
			mThisThreadHandler.sendEmptyMessage(MESSAGE_ICON_LOADED);
			return true;
		}

		private void readPic(ImageRequest id, ImageHolder holder, String path,
				boolean isVideo) {
			if (id.mId == 0) {
				id.mId = getDbId(id.mPath.toString(), isVideo);
				if (id.mId != 0) {
					Bitmap bitmap = getImageThumbnail(id.mId);
					if (bitmap != null) {
						holder.setImage(bitmap);
					}
				} else if (!isVideo) {
					Bitmap image = Utillocal.getImage(path.toString(), null,
							mContext);
					if (image != null) {
						holder.setImage(new BitmapDrawable(image));
					}
				}
			}
		}
