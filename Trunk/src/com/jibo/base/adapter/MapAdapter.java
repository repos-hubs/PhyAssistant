...
public class MapAdapter extends BaseAdapter {
	...
	public void addItemDataSrcList(List<? extends Object> objs) {
		if (this.getItemDataSrc() == null
			...
			this.setItemDataSrc(new AdapterSrc(objs));
		} else {
	...
	private void deployListeners(ActionListener[] actionListeners) {
		...
		for (ActionListener listener : actionListeners) {
			if (!listenerMaps.containsKey(listener.getResrcId())) {
				...
				listenerMaps.get(listener.getResrcId()).addActionListener(
						listener);
			}
		}
	}
	...
	public Object getItem(int position) {
		...
		return (itemDataSrc == null || itemDataSrc.getCount() < position) ? null
				: itemDataSrc.getItem(position);
...
public Set<Integer> continues = new HashSet<Integer>();
	protected void getViewInDetail(Object item, int position, View convertView) {
		...
		} else if (item instanceof EntityObj) {
			treatMap(((EntityObj) item).fieldContents, convertView, position);
		} else {
			treatObject(item, convertView, position);
		}
		if(position ==this.getCount()-1){
			if(!continues.contains(this.getCount()-1)){
				continues.add(this.getCount()-1);
				itemDataSrc.nextBatch(item);
			}
		}
	}
	...
	public static class AdaptInfo {
		...
		public AdapterSrc listviewItemData;// actual data-carried object
		public String[] objectFields;// data fields in sequence which map to
										// view ids
		public int[] viewIds; // id array in listview item
		public int listviewItemLayoutId;// layout id for each item in listview
		public ActionListener[] actionListeners;// varied action listeners in
