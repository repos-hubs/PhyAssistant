...
public class UpdateUserAllInfoPaser extends SoapDataPaser {
	...
	public void paser(SoapSerializationEnvelope response) {
		...
		SoapObject detail = (SoapObject) result
				.getProperty(SoapRes.RESULT_PROPERTY_UPDATE_USER_ALL_INFO);
		...
		for (int i = 0; i < detail.getPropertyCount(); i++) {
			...
			if (propertyInfo.name.equals("rescode")) {
				...
				login.setJob(soapChilds.getProperty("job").toString().equals(nullTag) ? "":soapChilds.getProperty("job").toString());
