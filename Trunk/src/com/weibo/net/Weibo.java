...
public class Weibo {
    ...
    public static String SERVER_QQ = "https://open.t.qq.com/api/";
    public static String SERVER_SINA = "https://api.weibo.com/2/";
    
    public static String URL_OAUTH2_ACCESS_AUTHORIZE_QQ = "https://open.t.qq.com/cgi-bin/oauth2/authorize";
    public static String URL_OAUTH2_ACCESS_AUTHORIZE_SINA = "https://api.weibo.com/oauth2/authorize";
    
    public static final int WEIBO_SINAL=0;
    ...
    public static final String DEFAULT_REDIRECT_URI = "wbconnect://success";// 闁哄棗鍊风粭澶愬绩椤栨稑鐦�
    public static final String DEFAULT_CANCEL_URI = "wbconnect://cancel";// 闁哄棗鍊风粭澶愬绩椤栨稑鐦�
...
//    }
    ...
    private void startDialogAuth(Activity activity, String[] permissions,int type) {
        WeiboParameters params = new WeiboParameters();
        if (permissions.length > 0) {
            params.add("scope", TextUtils.join(",", permissions));
        }
        CookieSyncManager.createInstance(activity);
        dialog(activity, params, type,new WeiboDialogListener() {
     ...
     * 
     * @param listener
     *            闁瑰搫鐗婂鍫㈢磼閹惧浜柣鈺傚灥閹宕抽敓锟�    */
    public void authorize(Activity activity, final WeiboDialogListener listener,int type) {
        authorize(activity, new String[] {}, DEFAULT_AUTH_ACTIVITY_CODE, listener,type);
    }

    private void authorize(Activity activity, String[] permissions,
            final WeiboDialogListener listener) {
        authorize(activity, permissions, DEFAULT_AUTH_ACTIVITY_CODE, listener,0);
    }

    private void authorize(Activity activity, String[] permissions, int activityCode,
            final WeiboDialogListener listener,int type) {
        Utility.setAuthorization(new Oauth2AccessTokenHeader());
        ...
        if (!singleSignOnStarted) {
            startDialogAuth(activity, permissions,type);
        }
    ...
    public void dialog(Context context, WeiboParameters parameters,int type,
            final WeiboDialogListener listener) {
		...
		case Weibo.WEIBO_SINAL:
	        ...
	        url = URL_OAUTH2_ACCESS_AUTHORIZE_SINA;
			break;
		case Weibo.WEIBO_QQ:
			parameters.add("client_id", APP_KEY);
	        parameters.add("response_type", "token");
	        parameters.add("redirect_uri", mRedirectUrl);
	        parameters.add("wap", "2");
	        url = URL_OAUTH2_ACCESS_AUTHORIZE_QQ;
			break;
		}
        ...
        url = url+ "?" + Utility.encodeUrl(parameters);
